...
export default {
  ...
  methods: {
    ...
    del() {
      ...
      this.$XModal.confirm('Are you sure to delete?').then((type) => {
        if (type === 'confirm') {
          ...
          axios.post('http://123.56.242.202:8080/api/BaseData/ProposalInstanceDelete?instanceID=' + checked.ID).then((res) => {
    ...
    getList(name) {
      axios.post('http://123.56.242.202:8080/api/BaseData/GetProposalInstance?departmentID=&userName=' + name).then((res) => {
