  ...
  <a-row>
    <a-col class="col" :span="24">
      <div class="toolbar">
        ...
        <a-button v-if="type !== 'children'" size="small" class="toolbtn" @click.stop="getGroup()">
          数据
        </a-button>
      </div>
      <div v-for="item in groupList.Then" :key="item.id" class="condition-item">
        <!-- <div
          v-if="groupList.Then.length > 1 && ['THEN', 'IF', 'ELSE'].includes(item.type)"
          class="line"
        ></div> -->
        <div class="condition-select" v-if="['fixed', 'forEach', 'IF', 'ELSE'].includes(item.type)">
          <div class="input-group">
            <span class="ifText Text" v-if="item.type == 'IF'">IF</span>
            <span class="elseText Text" v-if="item.type == 'ELSE'">ELSE IF</span>
          ...
          <a-select v-if="SeparateType == 3" v-model="item.compareOperation" class="symbol">
          ...
          >
            <a-select-option v-for="item in symbolOptions" :key="item.value" :value="item.value" :disabled="!isEdit">
              {{ item.label }}
            </a-select-option>
          </a-select>
          <div class="input-group">
            <a-input-group compact>
              ...
              <a-input v-model="item.rightOperatorExpression.value" class="selectCondition" placeholder="请选择" />
            </a-input-group>
          </div>
          <a-icon
        ...
        <div class="group" v-if="['IF', 'ELSE'].includes(item.type)">
          <div class="group-box">
            <div class="group-line"></div>
            <div class="group-content">
              <rule-blocks v-on="$listeners" :group-list="item" :isEdit="isEdit" type="children" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    selectBULoadData(item) {
      ...
      item[0].loading = true
      axios.get(`http://47.103.127.217:8080/api/SplitRule/GetSubBuList?departmentID=${item[0].ID}`).then(res => {
