          ...
          <a-col :xl="24" :lg="24" :md="24" :sm="24" :xs="24">
            <a-card :style="{ marginTop: '12px',height:'auto' }">
              <a-checkbox-group @change="designNameChange" v-model="designNameValue" style="width: 100%">
                <a-row>
                  <template v-for="(des, i) in designNameOption">
                    <a-col :span='6' :key="i">
...
<script>
  import deptRoleUserApi from '@/api/masterdata/deptRoleUser'
  ...
  export default {
    ...
    methods: {
      ...
      edit(record) {
        ...
        this.visible = true;
        deptRoleUserApi.getDeptRoleByUserId({userId:this.userId,deptId:this.currentDeptId}).then((res) => {
          if (res.isSuccess) {
            var designName = [];
            for (let value of res.data) {
              designName.push(value.deptRoleId)
            }
      ...
      handleSubmit() {
        ...
        deptRoleUserApi.deptRoleUserAdd(formData).then((res) => {
          if (res.isSuccess) {
      ...
      loadDesformList() {
        deptRoleUserApi.getDeptRoleList({ deptId: this.currentDeptId, userId:this.userId }).then((res) => {
          if (res.isSuccess) {
