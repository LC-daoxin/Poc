  ...
  <div class="activities">
    ...
    >
      <a-form-model-item
        ...
        label="Template"
      >
        <a-select allowClear :disabled="ActivityLoading" v-model="Template" @change="changeTemplate">
          <a-select-option value="EN">EN-WuXi XDC ADC CMC Development and Manufacturing for POC</a-select-option>
          <a-select-option value="CN"
            >CN-XDC CMC Development and Manufacturing Contract Service Proposal for POC</a-select-option
          >
        </a-select>
      </a-form-model-item>
      <a-form-model-item
        ...
        label="Activity"
      >
        <a-select allowClear :disabled="ActivityLoading" v-model="model.Activity" @change="changeActivity">
          <a-select-option :value="item.ActivityID" :key="item.ActivityID" v-for="item in ActivityOptions">{{
      ...
      <a-form-model-item
        ...
        label="SubActivity"
      >
        <a-select allowClear v-model="model.SubActivity" :disabled="SubActivityLoading" @change="changeSubActivity">
          <a-select-option :value="item.SubActivityID" :key="item.SubActivityID" v-for="item in SubActivityOptions">{{
      ...
      <a-form-model-item
        ...
        label="Version"
      >
        <a-select allowClear v-model="model.Version">
          <a-select-option :value="item.value" :key="item.value" v-for="item in VersionOptions">{{
            item.label
          }}</a-select-option>
        </a-select>
      </a-form-model-item>
    ...
    <div class="search-approval-wrapper">
      ...
      <a-button class="search_btn" type="primary" icon="plus" @click="addActivity">Add Row</a-button>
    ...
    ></public-table>
    <edit-rule-modal ref="ruleModal" :typeOptions="typeOptions" />
  </div>
</template>
  ...
  methods: {
    ...
    loadData() {
      if (this.model.SubActivity) {
        ...
        axios
          .post(
            `http://47.103.127.217:8080/api/BaseData/GetSplitRuleData?activityID=${this.model.SubActivity}&language=${this.Template}`
          )
          .then(res => {
    ...
    addActivity() {
      if (this.model.SubActivity) {
        axios
          .post('http://47.103.127.217:8080/api/BaseData/SplitRuleCreate', [
            {
    ...
    changeSubActivity(val) {
      ...
      this.tableParams.loading = true
      axios
        .post(`http://47.103.127.217:8080/api/BaseData/GetSplitRuleData?activityID=${val}&language=${this.Template}`)
        .then(res => {
    ...
    updateSplitList(record) {
      axios
        .post('http://47.103.127.217:8080/api/BaseData/SplitRuleUpdate', [
          {
            ...record
          }
        ])
    ...
    deleteRule(record) {
      console.log(record)
      axios.post(`http://47.103.127.217:8080/api/BaseData/SplitRuleDelete?ruleID=${record.ID}`).then(res => {
