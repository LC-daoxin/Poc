...
export default {
  ...
  methods: {
    ...
    getList(val) {
      ...
      axios.post('http://47.103.127.217:8080/api/user/GetMasterData?projectName=&corporateName=').then(res => {
    ...
    Approved1(row) {
      this.$XModal
        ...
        .then(type => {
          if (type === 'confirm') {
            this.$XModal.message({ id: 'loding', content: 'Data processing...', status: 'loading' })
            axios.post('http://47.103.127.217:8080/api/user/MasterDataUpdate?masterID=' + row.MasterID).then(res => {
              this.$XModal.close('loding')
              if (res.data.Code == 200) {
